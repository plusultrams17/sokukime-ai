...
3. 上記を組み合わせた完成トーク例文（preview: 挨拶→褒め→2度褒め→前提設定の流れ、150文字以上）
...
【基本3技術】
1. カギカッコ（第三者話法）
   - quote1: お客様の声①（「もっと早くやればよかった」等、カギカッコ付きでそのまま使えるセリフ）
   - quote2: お客様の声②（別の切り口のポジティブな声）
2. 社会的証明（過半数）
   - socialProof: 「みなっっっさん」を強調した過半数フレーズ（「みなっさんが選ばれている○○をぜひ！」形式）
3. 一貫性通し
   - consistency: 前提条件の振り返り＋訴求（「最初にお伝えした通り、良いと思ったらスタートしてください。」形式）

【ポジティブクロージング】
4. ポジティブシングル（SP→ベネフィット→天国想像→カギカッコ→訴求）
...
9. 訴求フレーズ（appealPhrase）：カギカッコ＋一貫性通しor過半数＋訴求の組み合わせ
...
【切り返し5技法】
2. 技法①「目的の振り返り」（考えたい・検討したいケースに有効）
   - tech1Recall: 目的の振り返り（「今日の目的は○○でしたよね？」→視野が狭くなった顧客を森に戻す）
   - tech1Area: AREA話法（A:「○○が目的であれば絶対今決断すべきです」R: 理由 E: 例え A: 繰り返し）

3. 技法②「第三者アタック」（考えたい・検討したいケースに有効）
   - tech2Episode: 第三者エピソード（天国or地獄のストーリー。多少長くてもOK、具体的に。自分の感情を入れてもOK）

4. 技法③「＋のシャワー」（バトルっぽく熱くなったケースに有効）
   - tech3YesPlus: YES+質問の流れ（「どのポイントが良いですか？」→「なぜそのポイントが良い？」→お客様自身にプラスを語らせる）

5. 技法④「すり替え」（他社比較・他者相談・今じゃなくてもいいに有効）
...
6. 技法⑤「価値の上乗せ」（価格が高いと言われたケースに有効）
   - tech5Apology: 驚き＋謝罪（「高いですか！高く思わせてしまって申し訳ございません。私の伝え方が悪かっただけなんです」）
   - tech5Value: SP+ベネフィット3連発で価値を再提示（「みなっさん安いよねーとおっしゃってくださる理由は〜」形式）

7. 完成された反論処理フロー例（preview：共感→感謝→フック→いずれかの技法→AREA→訴求の流れ、250文字以上）

JSON形式で返してください：
{ "empathy": "...", "gratitude": "...", "hook": "...", "tech1Recall": "...", "tech1Area": "...", "tech2Episode": "...", "tech3YesPlus": "...", "tech4Reframe": "...", "tech4Correction": "...", "tech5Apology": "...", "tech5Value": "...", "preview": "..." }`,
};
